...
from scipy.sparse import issparse   #JAIME!!!!!!
...
class CV_estimator(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        """ Fit the models
            ...
            I also stores some attributes needed by `predict` and `predict_proba`, namely, le_, classes_ X_train_
            and y_train_
    ...
    def _predict_proba(self, X):
        ...
        """
        n_examples = X.shape[0]                          #JAIME!!!!!
        if issparse(X):                                  #JAIME!!!!!
            equals = (X!=self.X_train_).nnz==0           #JAIME!!!!!
        else:                                            #JAIME!!!!!
            equals = np.array(X == self.X_train_).all()  #JAIME!!!!!
        if equals:                                       #JAIME!!!!!
